...
def draw_epipolar_imgs(image0, image1, R, t, intrinsics):
    # image0, image1: (3, H, W) [0, 1]
    image0 = (image0.permute(1, 2, 0) * 255).int()[..., [2, 1, 0]].numpy()
    image1 = (image1.permute(1, 2, 0) * 255).int()[..., [2, 1, 0]].numpy()

    K = torch.zeros(1,3,3).double()
    K[0,0,0] = intrinsics[0]
    K[0,1,1] = intrinsics[1]
    K[0,0,2] = intrinsics[2]
    K[0,1,2] = intrinsics[3]
    K[0,2,2] = 1
    ...
    for y1 in np.arange(startx, stopx, stepx):
        for y2 in np.arange(starty, stopy, stepy):
            ...
            m, b = get_epipolar(y, torch.from_numpy(rot_mtx), K)
...
def apply_transform(mtx, transform):
